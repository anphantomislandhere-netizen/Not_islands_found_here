...
def gerar_mapa_calor(grade_lattice, grade_necessidade, cx_min, cz_min,
    ...
    # --- PAINEL ESQUERDO: lattice_max (AGORA COM A REGRA DO AZUL PARA > 0) ---
    cmap_lattice = plt.get_cmap("RdYlGn")
    norm_lattice = Normalize(vmin=-100, vmax=50)
    rgba_lattice = cmap_lattice(norm_lattice(grade_lattice))
    mascara_positiva_lattice = grade_lattice > 0
    rgba_lattice[mascara_positiva_lattice] = to_rgba("blue")

    axes[0].imshow(rgba_lattice.transpose(1, 0, 2), origin="lower",
                   extent=[cx_min, cx_min + largura, cz_min, cz_min + altura])
    axes[0].set_title("lattice_max — checagem REAL (azul = qualquer valor >0)")
    axes[0].set_xlabel("chunk X")
    axes[0].set_ylabel("chunk Z")
    sm_lat = plt.cm.ScalarMappable(cmap=cmap_lattice, norm=norm_lattice)
    sm_lat.set_array([])
    fig.colorbar(sm_lat, ax=axes[0], label="densidade máxima — azul = >0")

    # --- PAINEL DIREITO: necessidade (MÉTRICA NOVA, VOLTOU AO NORMAL) ---
...
if __name__ == "__main__":
    ...
    for indice, (x_ramo, z_ramo) in enumerate(ramos, start=1):
        ...
        gerar_mapa_calor(grade_lattice, grade_necessidade, cx_centro - RAIO, cz_centro - RAIO,
                         ilhas_conhecidas=marcador, caminho_saida=f"mapa{indice}.png")
        exportar_csv(grade_lattice, grade_necessidade, grade_macro,
                     cx_centro - RAIO, cz_centro - RAIO, caminho=f"grade{indice}.csv")
